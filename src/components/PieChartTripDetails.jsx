...
const PieChartTripDetails = ({ data }) => {
  const theme = useTheme();
  const colors = tokens(theme.palette.mode);

  const margin = { top: 50, right: 0, bottom: 50, left: 0 };
  const totalMovingTime = data.movingTime;

  const totalTrafficTime = data.trafficTime;
  const totalIdleTime = data.idleTime;
...
export default PieChartTripDetails;
